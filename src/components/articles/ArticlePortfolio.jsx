...
function ArticlePortfolio({ data }) {
  ...
  const [expandableFilterResult, setExpandableFilterResult] =
    useState(parsedItems)
  const [shouldAnimate, setShouldAnimate] = useState(false)
  const [skipAnimationForNextChange, setSkipAnimationForNextChange] =
    useState(false)

  useEffect(() => {
    const parsedItems = parser.parseArticleItems(parsedData.items)
    const parsedCategories = parser.parseArticleCategories(
  ...
  const _getMaxItemsPerPage = () => {
    if (isBreakpoint('xxl')) return 9
    if (isBreakpoint('lg')) return 8
    if (isBreakpoint('sm')) return 6
    return 4
  }

  //   console.log('expandableFilterResult', expandableFilterResult)
        ...
        >
          ...
          >
            {expandableFilterResult.map((item, key) => (
              <Col
                key={key}
                className={`col-12 col-sm-12 col-md-12 col-lg-12 col-xxl-12`}
              >
                <ProjectCard
                  title={item.title}
